...
def generate_code_prompt(use_example_for=None, run_in_simple_mode=False):
    """
    ...
    '''
    if run_in_simple_mode:
        # force it not to use template
        use_example_for = None

    if use_example_for is not None:
        code_example = read_code_example_for(use_example_for)
        if code_example is not None:
            example_based_instruction = f'''
    
    To generate the code, please use the example code below and finish the TODOs. 
    If necessary, make proper adjustments. 
    
    {code_example.replace('{', '{{').replace('}', '}}')}
            '''
            func_name_instruction = '[Note] please do use the specified function name `{fun_name}` but not the function name in the example code.'
            return basic_instruction + example_based_instruction + func_name_instruction
...
"""
    ...
    **Support** measures how many rows in the dataset are involved in the rule, relative to the total number of rows in the dataset. It is calculated as:
    ...
    ### Output Specification:
     Your response should return the following variable:
     - program: a program containing the function code. The code should be a single-line string with escaped newlines and quotes.
       For example:
       "program": "import pandas as pd\\ndef {fun_name}(dataframe):\\n    # Implement the logic here\\n    ...\\n    return {{"support": support, "confidence": confidence, "satisfactions": satisfactions,"violations": violations}}"
    '''
